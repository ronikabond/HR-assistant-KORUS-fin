-- Раньше сброс демо-пространства нарочно обходил триггер онбординга для ВСЕХ
-- пятерых аккаунтов (связи hr_id/manager_id проставлялись отдельным UPDATE уже
-- после вставки профиля, чтобы триггер private.k_seed_onboarding_meetings не
-- сработал). Из-за этого Илья и Дарья — единственные два аккаунта, которые по
-- сюжету демо реально «только что вышли на работу» — не получали ни стандартный
-- график адаптации, ни дедлайн «Подготовить ИПР» для руководителя.
--
-- Теперь Илья и Дарья вставляются сразу со связями (как обычный новый
-- сотрудник через форму/CSV-импорт), и триггер создаёт им график сам —
-- ровно так же, как для любого другого нового сотрудника. Иван, Олег и Елена
-- по-прежнему вставляются без связей: они играют роль уже работающих
-- сотрудников, им онбординг не нужен.
...
begin
  ...
  -- Иван, Олег и Елена — уже работающие сотрудники по сюжету демо. Вставляем
  -- без hr_id/manager_id, чтобы триггер онбординга не сработал для них.
  ...
  where account.key in ('head','hr','manager');
  ...
  -- Илья и Дарья вставляются сразу со связями — все прошлые встречи уже
  -- удалены выше, так что триггер private.k_seed_onboarding_meetings создаёт
  -- им реальный график адаптации (и дедлайн «Подготовить ИПР» руководителю)
  -- ровно так же, как для любого нового сотрудника, заведённого через форму.
  insert into public.k_profiles(
    id,login,full_name,job_title,department,direction,corporate_email,phone,
    hired_on,is_hr,is_head_hr,is_active,hr_id,manager_id
  )
  select
    (account.value->>'id')::uuid,
    account.value->>'login',
    ...
    coalesce((account.value->>'is_head_hr')::boolean,false),
    true,
    case account.key when 'ilya' then hr_id when 'darya' then head_id end,
    manager_id
  from jsonb_each(p_accounts) account
  where account.key in ('ilya','darya');
