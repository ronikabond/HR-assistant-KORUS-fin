...
export async function sendOneTimeSurvey(template:Omit<SurveyTemplate,'id'>,subject:Profile,audience:'person'|'colleagues'|'all') { const{error}=await client().rpc('k_send_one_time_survey',{p_kind:template.kind,p_title:template.title,p_description:template.description,p_questions:template.questions.map((q,index)=>({text:q.text,answer_type:q.answer_type,position:index+1})),p_subject_id:subject.id,p_audience:audience});fail(error) }
