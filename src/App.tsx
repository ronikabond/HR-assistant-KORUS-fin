...
export default function App(){
  ...
  const page=()=>{
    ...
    switch(view){
      case'home':return <HomePage me={me} data={data} calendar={calendar(true)} ipr={ipr(me,true)}/>
...
function NotificationsPanel({notifications,meetings,profiles,meId,onClose,onRead,onReadAll,onDelete}:{notifications:api.WorkspaceData['notifications'];meetings:Meeting[];profiles:Profile[];meId:string;onClose:()=>void;onRead:(id:number)=>Promise<void>;onReadAll:()=>Promise<void>;onDelete:(id:number)=>Promise<void>}){const[detail,setDetail]=useState<{title:string;body:string;date:string;kind:'meeting'|'notification'}|null>(null);const reminders=useMemo(()=>meetings.filter((meeting)=>{if(meeting.meeting_type==='deadline')return false;const days=(new Date(meeting.scheduled_for).getTime()-Date.now())/86400000;return days>=0&&days<=7}),[meetings]);return <div className="drawer-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&onClose()}><aside className="notification-drawer"><div className="drawer-head"><div><h2>Уведомления</h2><p>Нажмите на уведомление, чтобы прочитать подробнее</p></div><button className="icon-button" onClick={onClose}><X/></button></div><button className="button secondary small read-all" onClick={()=>void onReadAll()}>Прочитать все</button><div className="notification-list">{reminders.map((meeting)=>{const date=new Date(meeting.scheduled_for).toLocaleString('ru-RU');const iprPrep=meeting.meeting_type==='probation_end'&&meeting.participant_roles?.[meId]==='manager';const title=iprPrep?'Подготовьте ИПР':'Встреча в течение недели';const body=iprPrep?`Нужно подготовить ИПР для сотрудника ${profiles.find((profile)=>profile.id===meeting.employee_id)?.full_name??''}`:meeting.title;return <article className="notification unread" role="button" tabIndex={0} key={`meeting-${meeting.id}`} onClick={()=>setDetail({title,body,date,kind:'meeting'})}><span className="notification-icon">{iprPrep?<ClipboardList/>:<CalendarDays/>}</span><div><strong>{title}</strong><p>{body}</p><small>{date}</small></div></article>})}{notifications.map((item)=>{const open=()=>{setDetail({title:item.title,body:item.body,date:new Date(item.created_at).toLocaleString('ru-RU'),kind:'notification'});if(!item.is_read)void onRead(item.id)};return <article className={`notification ${!item.is_read?'unread':''}`} role="button" tabIndex={0} key={item.id} onClick={open} onKeyDown={(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open()}}}><span className="notification-icon"><Bell/></span><div><strong>{item.title}</strong><p>{item.body}</p><small>{new Date(item.created_at).toLocaleString('ru-RU')}</small></div>{canDeleteNotification(item,meetings)&&<button className="icon-button" aria-label="Удалить уведомление" onClick={(event)=>{event.stopPropagation();void onDelete(item.id)}}><X/></button>}</article>})}{!reminders.length&&!notifications.length&&<EmptyState icon={<Bell/>} title="Пока тихо" text="Новые события появятся здесь."/>}</div></aside>{detail&&<Modal title={detail.title} subtitle={detail.kind==='meeting'?'Предстоящая встреча':'Уведомление'} onClose={()=>setDetail(null)} wide><div className="notification-detail"><span className="notification-detail-icon">{detail.kind==='meeting'?<CalendarDays/>:<Bell/>}</span><p>{detail.body}</p><time>{detail.date}</time><button className="button primary" onClick={()=>setDetail(null)}>Понятно</button></div></Modal>}</div>}
