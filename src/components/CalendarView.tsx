...
import { CalendarDays, ChevronLeft, ChevronRight, Clock, Info, Plus, X } from 'lucide-react'
...
export function CalendarView({me,profiles,meetings,canCreate,readOnly=false,onCreate,onReschedule,onRespond}:{me:Profile;profiles:Profile[];meetings:Meeting[];canCreate:boolean;readOnly?:boolean;onCreate:(v:{title:string;employee_id:string;scheduled_for:string;participant_ids:string[];participant_roles:Record<string,MeetingParticipationRole>})=>Promise<void>;onReschedule:(meeting:Meeting,date:string,reason:string)=>Promise<void>;onRespond:(meeting:Meeting,status:'accepted'|'declined')=>Promise<void>}){
  ...
  return <section className="card calendar-card">
    ...
    {selected&&<Modal className="meeting-modal" title={selected.title} subtitle={format(new Date(selected.scheduled_for),'d MMMM yyyy, HH:mm',{locale:ru})} onClose={()=>{setSelected(null);setReschedule(false)}}>
...
function RescheduleForm({meeting,onSubmit}:{meeting:Meeting;onSubmit:(date:string,reason:string)=>Promise<void>}){const[date,setDate]=useState(meeting.scheduled_for.slice(0,16));const[reason,setReason]=useState('');return <form className="form-stack" onSubmit={(e)=>{e.preventDefault();void onSubmit(new Date(date).toISOString(),reason)}}><label>Новая дата и время<DateTimeField type="datetime-local" value={date} onChange={setDate} required ariaLabel="Новая дата и время встречи"/></label><label>Причина<textarea value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="Коротко поясните причину"/></label><button className="button primary">Отправить HR</button></form>}
function CreateMeeting({profiles,me,onClose,onSubmit}:{profiles:Profile[];me:Profile;onClose:()=>void;onSubmit:(v:{title:string;employee_id:string;scheduled_for:string;participant_ids:string[];participant_roles:Record<string,MeetingParticipationRole>})=>Promise<void>}){const[title,setTitle]=useState('Личная встреча');const[date,setDate]=useState(new Date(Date.now()+86400000).toISOString().slice(0,16));const[participants,setParticipants]=useState<string[]>([]);const submit=()=>onSubmit({title,employee_id:me.id,scheduled_for:new Date(date).toISOString(),participant_ids:participants,participant_roles:Object.fromEntries([me.id,...participants].map((id)=>[id,id===me.id?'employee':'participant']))});return <Modal className="meeting-create-modal" title="Новая встреча" subtitle="Приглашённые смогут принять встречу, отказаться или предложить другую дату" onClose={onClose} wide><form className="form-stack" onSubmit={(e)=>{e.preventDefault();void submit()}}><label>Название<input value={title} onChange={(e)=>setTitle(e.target.value)} required/></label><label>Дата и время<DateTimeField type="datetime-local" value={date} onChange={setDate} required ariaLabel="Дата и время новой встречи"/></label><fieldset><legend>Пригласить сотрудников</legend><ProfilePicker profiles={profiles.filter((p)=>p.id!==me.id)} selectedIds={participants} onChange={setParticipants} multiple/></fieldset><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Отмена</button><button className="button primary">Отправить приглашения</button></div></form></Modal>}
