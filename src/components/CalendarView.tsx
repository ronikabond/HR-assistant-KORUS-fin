...
export function CalendarView({me,profiles,meetings,canCreate,readOnly=false,onCreate,onReschedule,onRespond,onOpenProfile}:{me:Profile;profiles:Profile[];meetings:Meeting[];canCreate:boolean;readOnly?:boolean;onCreate:(v:{title:string;employee_id:string;scheduled_for:string;participant_ids:string[];participant_roles:Record<string,MeetingParticipationRole>})=>Promise<void>;onReschedule:(meeting:Meeting,date:string,reason:string)=>Promise<void>;onRespond:(meeting:Meeting,status:'accepted'|'declined')=>Promise<void>;onOpenProfile?:(profile:Profile)=>void}){
  ...
  return <section className="card calendar-card">
    ...
    <div className="calendar-grid"><div className="weekdays">{['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map((d)=><span key={d}>{d}</span>)}</div><div className="calendar-days">{days.map((day)=>{const dayMeetings=meetings.filter((meeting)=>isSameDay(new Date(meeting.scheduled_for),day)&&meeting.participant_statuses?.[me.id]!=='declined');const visibleLimit=dayMeetings.length>4?3:4;const hidden=dayMeetings.slice(visibleLimit);return <div key={day.toISOString()} className={`day ${!isSameMonth(day,month)?'outside':''} ${isToday(day)?'today':''}`}><time>{format(day,'d')}</time><div className="day-events">{dayMeetings.slice(0,visibleLimit).map((meeting)=><button key={meeting.id} title={`${meeting.title} · ${format(new Date(meeting.scheduled_for),'d MMMM, HH:mm',{locale:ru})}`} className={`meeting-pill ${meetingColor(meeting,me)}`} onClick={()=>setSelected(meeting)}><span>{format(new Date(meeting.scheduled_for),'HH:mm')}</span>{meeting.title}</button>)}{hidden.length>0&&<span className="meeting-overflow" title={hidden.map((meeting)=>meeting.title).join('\n')}>Ещё {hidden.length}</span>}</div></div>})}</div></div>
