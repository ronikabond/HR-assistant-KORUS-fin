...
function LinkForm({profiles,item,onClose,onSubmit}:{profiles:Profile[];item?:ResourceLink;onClose:()=>void;onSubmit:(values:{title:string;url:string;description:string},share:ShareDraft)=>Promise<void>}){const[values,setValues]=useState({title:item?.title??'',url:item?.url??'https://',description:item?.description??''});const[share,setShare]=useState<ShareDraft>({access_scope:item?.access_scope??'office',department:item?.department??null,recipient_ids:item?.recipient_ids??[]});const[busy,setBusy]=useState(false);const submit=async()=>{if(busy)return;setBusy(true);try{await onSubmit(values,share)}finally{setBusy(false)}};return <Modal title={item?'Изменить ссылку':'Новая полезная ссылка'} onClose={onClose} wide><form className="form-stack" onSubmit={(e)=>{e.preventDefault();void submit()}}><label>Название<input value={values.title} onChange={(e)=>setValues({...values,title:e.target.value})} disabled={busy} required/></label><label>Адрес<input type="url" value={values.url} onChange={(e)=>setValues({...values,url:e.target.value})} disabled={busy} required/></label><label>Описание<textarea value={values.description} onChange={(e)=>setValues({...values,description:e.target.value})} disabled={busy}/></label><AccessEditor profiles={profiles} value={share} onChange={setShare}/><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose} disabled={busy}>Отмена</button><button className="button primary" disabled={busy}>{busy?'Сохраняем…':'Сохранить'}</button></div></form></Modal>}
