...
function TakeSurvey({run,template,subject,onClose,onSubmit}:{run:SurveyRun;template:SurveyTemplate;subject:Profile;onClose:()=>void;onSubmit:(values:Record<number,string>)=>Promise<void>}){
  ...
  const submit=async()=>{if(busy)return;setBusy(true);try{await onSubmit(values)}finally{setBusy(false)}}
  return <Modal className="survey-take-modal" title={template.title} subtitle={`О сотруднике: ${subject.full_name}`} onClose={onClose} wide><form className="survey-form" onSubmit={(event)=>{event.preventDefault();void submit()}}>{template.questions.map((question,index)=><div className="survey-question" key={question.id}><span>Вопрос {index+1} из {template.questions.length}</span><h3>{question.text}</h3>{question.answer_type==='scale'?<div className="scale-answer">{Array.from({length:10},(_,scaleIndex)=>scaleIndex+1).map((number)=><label key={number}><input type="radio" name={`q-${question.id}`} value={number} onChange={()=>setValues({...values,[question.id!]:String(number)})} disabled={busy} required/><span>{number}</span></label>)}</div>:<textarea value={values[question.id!]??''} onChange={(event)=>setValues({...values,[question.id!]:event.target.value})} placeholder="Ваш ответ" disabled={busy} required/>}</div>)}<button className="button primary" disabled={busy}>{busy?'Отправляем…':'Отправить ответы'}</button></form></Modal>
}
...
function EditTemplate({template,initialEditing=false,onClose,onSubmit}:{template:SurveyTemplate;initialEditing?:boolean;onClose:()=>void;onSubmit:(template:SurveyTemplate)=>Promise<void>}){
  ...
  const save=async()=>{if(saving||!hasChanges||!isValid)return;setSaving(true);try{await onSubmit(draft)}finally{setSaving(false)}}
  return <Modal className="survey-template-modal" title={isEditing?'Редактировать шаблон':'Шаблон опроса'} subtitle={isEditing?'Изменения применятся к следующим отправкам':'Просмотр сохранённой версии шаблона'} onClose={onClose} wide><div className={`form-stack template-mode ${isEditing?'is-editing':'is-viewing'}`}><label>Название<input value={draft.title} readOnly={!isEditing} disabled={saving} onChange={(event)=>setDraft({...draft,title:event.target.value})}/></label><label>Описание<textarea value={draft.description} readOnly={!isEditing} disabled={saving} onChange={(event)=>setDraft({...draft,description:event.target.value})}/></label><div className={`question-editor ${isEditing?'is-editing':''}`}>{draft.questions.map((question,index)=><div key={question.id??index}><b>{index+1}</b><input value={question.text} readOnly={!isEditing} disabled={saving} onChange={(event)=>update(index,{text:event.target.value})}/><SelectField value={question.answer_type} options={answerTypeOptions} disabled={!isEditing||saving} onChange={(answerType)=>update(index,{answer_type:answerType as 'text'|'scale'})} ariaLabel={`Тип ответа на вопрос ${index+1}`}/>{isEditing&&<button type="button" className="icon-button danger question-delete" disabled={saving||draft.questions.length===1} onClick={()=>remove(index)} aria-label={`Удалить вопрос ${index+1}`} title={draft.questions.length===1?'В шаблоне должен остаться хотя бы один вопрос':'Удалить вопрос'}><Trash2/></button>}</div>)}</div>{isEditing&&<button type="button" className="button add-line" disabled={saving} onClick={()=>setDraft({...draft,questions:[...draft.questions,{text:'Новый вопрос',answer_type:'text',position:draft.questions.length+1}]})}><Plus/>Добавить вопрос</button>}<div className="template-mode-actions">{isEditing?<><button type="button" className="button secondary" disabled={saving} onClick={cancelEditing}>Отменить изменения</button><button type="button" className="button primary" disabled={saving||!hasChanges||!isValid} onClick={()=>void save()}>{saving?'Сохраняем…':'Сохранить изменения'}</button></>:<button type="button" className="button primary" onClick={()=>setIsEditing(true)}><Edit3/>Редактировать шаблон</button>}</div></div></Modal>
}
