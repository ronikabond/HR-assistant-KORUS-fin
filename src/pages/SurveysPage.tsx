...
import { BarChart3, CalendarClock, CheckCircle2, ClipboardList, Edit3, Plus, Save, Send, Trash2 } from 'lucide-react'
...
const emptyDraft=():SurveyDraft=>({kind:'custom',title:'',description:'',questions:[{text:'',answer_type:'text',position:1}]})
...
export function SurveysPage({me,profiles,templates,runs,assignments,answers,schedules,onSend,onSendOneTime,onSubmit,onSaveTemplate,onCreateTemplate,onDeleteTemplate,onCreateSchedule,onDeleteSchedule}:SurveysPageProps){
  ...
  return <>
    ...
    {me.is_hr&&<>
      <div className="survey-mode-switch" role="tablist" aria-label="Режим работы с опросами">
        <button type="button" role="tab" aria-selected={mode==='create'} className={mode==='create'?'active':''} onClick={()=>setMode('create')}><Plus/>Создать новый</button>
        <button type="button" role="tab" aria-selected={mode==='templates'} className={mode==='templates'?'active':''} onClick={()=>setMode('templates')}><ClipboardList/>Шаблоны <Badge tone="purple">{templates.length}</Badge></button>
      </div>

      {mode==='create'?<NewSurveyBuilder key={builderVersion} onSave={async(template)=>{await onCreateTemplate(template);setBuilderVersion((version)=>version+1);setMode('templates')}} onSend={setOneTimeDraft}/>:<TemplateLibrary templates={templates} onOpen={setEditing} onSend={setSending} onDelete={onDeleteTemplate}/>}
    ...
    {editing&&<EditTemplate template={editing} onClose={()=>setEditing(null)} onSubmit={async(template)=>{await onSaveTemplate(template);setEditing(null)}}/>}
...
function TemplateLibrary({templates,onOpen,onSend,onDelete}:{templates:SurveyTemplate[];onOpen:(template:SurveyTemplate)=>void;onSend:(template:SurveyTemplate)=>void;onDelete:(id:number)=>Promise<void>}){
  if(!templates.length)return <section className="card survey-library-empty"><EmptyState icon={<ClipboardList/>} title="Шаблонов пока нет" text="Создайте опрос и сохраните его в шаблоны — он появится здесь."/></section>
  return <section aria-label="Шаблоны опросов"><div className="template-grid">{templates.map((template)=><article className="template-card" key={template.id}><div className={`template-icon ${template.kind}`}><ClipboardList/></div><Badge tone={template.kind==='self'?'blue':'purple'}>{template.kind==='self'?'Опрос сотрудника':template.kind==='colleagues'?'Опрос коллег':'Свой шаблон'}</Badge><h3>{template.title}</h3><p>{template.description}</p><span>{template.questions.length} вопросов</span><div className="template-actions"><button className="button secondary small" onClick={()=>onOpen(template)}><Edit3/>Открыть</button><button className="button primary small" onClick={()=>onSend(template)}><Send/>Отправить</button><button className="icon-button danger" onClick={()=>void onDelete(template.id)} title="Удалить шаблон" aria-label={`Удалить шаблон ${template.title}`}><Trash2/></button></div></article>)}</div></section>
}

function NewSurveyBuilder({onSave,onSend}:{onSave:(template:SurveyDraft)=>Promise<void>;onSend:(template:SurveyDraft)=>void}){
  ...
  return <section className="card survey-builder-card">
    <div className="survey-builder-head"><div><span className="eyebrow">Новый опрос</span><h2>Создать с нуля</h2><p>Сохраните опрос для повторного использования или отправьте его один раз.</p></div><div className="survey-symbol"><Plus/></div></div>
...
function EditTemplate({template,onClose,onSubmit}:{template:SurveyTemplate;onClose:()=>void;onSubmit:(template:SurveyTemplate)=>Promise<void>}){
